  ...
  if (!standard) {
    return (
      <Link
        ...
        className="group block rounded-2xl border border-dashed border-azure-blue/20 bg-light-sky/25 p-6 transition-all hover:border-azure-blue/40 hover:bg-light-sky/45 hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-turquoise-sea focus-visible:ring-offset-2"
      >
        <p className="text-xs font-semibold uppercase tracking-[0.15em] text-azure-blue/70">
          {playbookTitle}
        </p>
        ...
        <p className="mt-5 inline-flex items-center gap-1 text-xs text-azure-blue group-hover:underline underline-offset-4">
          Open playbook
      ...
      {qualities.length > 0 ? (
        <ul className="mt-4 flex flex-wrap gap-1.5">
          {qualities.map((q, i) => (
            <li
              key={i}
              className="rounded-full bg-white border border-orange-peel/30 px-2.5 py-1 text-xs text-azure-blue"
            >
              {q}
            </li>
          ))}
